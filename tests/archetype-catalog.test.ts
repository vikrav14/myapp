...
describe("archetype catalog", () => {
  it("maps numeric replies to the documented 1–5 order", () => {
    ...
    expect(inferArchetypeFromMessage("5")).toBe("Custom");
  });
  ...
  it("maps text aliases to archetypes", () => {
    ...
    expect(inferArchetypeFromMessage("custom")).toBe("Custom");
    expect(inferArchetypeFromMessage("my own mix")).toBe("Custom");
  });
});
